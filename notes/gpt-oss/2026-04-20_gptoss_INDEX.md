...
| **28** | [`2026-05-07_gptoss_28_b200_baseline_step17.md`](./2026-05-07_gptoss_28_b200_baseline_step17.md) | **B200 step #17 baseline（rank2+rank3 平均）+ note 16 硬件标错更正**：原 trace 是 `gpt_oss_20b_nvidia` × 8（B200 / sm100），note 16 头注的 "MI355X × 8" 是错的，§"一手数据" 那张 Top-25 表已废弃。8 rank step 17 wall = **966.75 ± 0.10 ms**，但 GPU 段只有 rank 2 / rank 3 完整（其它 6 个 trace 文件被截断在 CPU 段，CUPTI buffer 没 flush），故以这两个 rank 平均做 baseline：SendRecv 224 ms (23%) + RS 192 + AG 154 + GEMM 141 + elementwise 88 + attn 64 + moeD 41 + norm 30 + Adam 14；compute-busy **48.5 %**，NCCL hidden **24.0 %**，stream oversub 106 %。**rank2 vs rank3 失衡显著**（GEMM 差 67 ms、SendRecv 差 120 ms），定性是 MoE per-expert token 不均；任何后续 A/B 必须 ≥ 2 个 rank 平均。**Tier 1A "fused residual+RMSNorm" 收益不可移植到 B200**（B200 没有那个 42 ms 独立 `vectorized_elementwise_kernel<bf16 add>`），Tier 1B "SwiGLU 去 cat" 跨硬件成立 |
